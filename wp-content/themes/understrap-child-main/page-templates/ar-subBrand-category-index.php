<?php
/**
 * Template Name: Arrow Sub Brand Category Index Page
 *
 *
 * @package arrow
 */
...
<h3 class="current-cat text-center pg-heading"> <?= $ar_active_cat->name  ?></h3>
<div class="loader">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function loadProductItems(limit, page, target = null, filter = null) {
    ...
    return fetch('<?php echo admin_url("admin-ajax.php"); ?>', {
        ...
        body: new URLSearchParams({
          ...
          page,
          filter,
          sub_brand_category_id: '<?= $ar_active_cat->id ?>',
  ...
  document.addEventListener('click', function(e) {
    ...
    if (e.target.closest('.product-items-count li')) {
      ...
      loadProductItems(limit, 1, e.target);
    ...
    if (e.target.closest('.page-item')) {
      ...
      const limit = document.querySelector('.product-items-count li.active').dataset.val;
      loadProductItems(limit, page, e.target);
    }
  ...
  //   loadProductItems(limit, page, e.target, filter);

  // })

  // document.querySelector('.prods_filters').style.display = 'none'
});
</script>